export const STOCKS_DATABASE = [
  ...
  { symbol: 'GOOGL', name: 'Alphabet Inc. (Class A)', exchange: 'NASDAQ', sector: 'Internet Services & AI', marketCap: '$2.15T', pe: 24.3, rating: 'Strong Buy', qualityScore: 95, analysisSnapshot: 'Search cash flow durability paired with Gemini LLM enterprise cloud growth.', keywords: ['google', 'alphabet', 'search', 'gemini', 'youtube', 'android', 'cloud'] },
  ...
  { symbol: 'GE', name: 'GE Aerospace', exchange: 'NYSE', sector: 'Aerospace Engines', marketCap: '$185B', pe: 38.0, rating: 'Strong Buy', qualityScore: 93, analysisSnapshot: 'LEAP commercial engine aftermarket servicing and robust defense backlog.', keywords: ['general electric', 'aerospace', 'engines'] },
  { symbol: 'GM', name: 'General Motors Company', exchange: 'NYSE', sector: 'Automotive & EVs', marketCap: '$52B', pe: 5.4, rating: 'Buy', qualityScore: 90, analysisSnapshot: 'High-margin internal combustion truck fleet funding Ultium EV platform transformation.', keywords: ['general motors', 'chevy', 'chevrolet', 'cadillac', 'gmc'] },
  { symbol: 'GS', name: 'The Goldman Sachs Group Inc.', exchange: 'NYSE', sector: 'Investment Banking & Wealth', marketCap: '$165B', pe: 16.2, rating: 'Strong Buy', qualityScore: 93, analysisSnapshot: 'Global premier advisory and capital markets franchise expanding asset management scale.', keywords: ['goldman sachs', 'banking', 'wealth management'] },
  { symbol: 'GILD', name: 'Gilead Sciences Inc.', exchange: 'NASDAQ', sector: 'Biotechnology & Virology', marketCap: '$98B', pe: 14.8, rating: 'Buy', qualityScore: 91, analysisSnapshot: 'Dominant HIV treatment market share paired with expanding oncology cell therapy pipeline.', keywords: ['gilead', 'biotech', 'hiv', 'oncology', 'biktarvy'] },
  { symbol: 'GIS', name: 'General Mills Inc.', exchange: 'NYSE', sector: 'Consumer Packaged Foods', marketCap: '$38B', pe: 15.5, rating: 'Hold', qualityScore: 89, analysisSnapshot: 'Defensive cash flow profile supported by iconic Cheerios, Häagen-Dazs, and Blue Buffalo pet brands.', keywords: ['general mills', 'cheerios', 'food', 'blue buffalo'] },
  { symbol: 'GRMN', name: 'Garmin Ltd.', exchange: 'NYSE', sector: 'Consumer & Aviation Electronics', marketCap: '$35B', pe: 24.5, rating: 'Buy', qualityScore: 92, analysisSnapshot: 'Premium fitness smartwatch brand loyalty and mission-critical aviation avionics dominance.', keywords: ['garmin', 'gps', 'smartwatch', 'aviation', 'marine'] },
  
  
  { symbol: 'GRASIM', name: 'Grasim Industries Limited', exchange: 'NSE', sector: 'Cement & Chemicals', marketCap: '₹1.72L Cr', pe: 38.4, rating: 'Hold', qualityScore: 88, analysisSnapshot: 'Aditya Birla Group holding company launching major decorative paints offensive.', keywords: ['grasim', 'aditya birla', 'cement', 'chemicals', 'birla opus'] },
  { symbol: 'GODREJCP', name: 'Godrej Consumer Products', exchange: 'NSE', sector: 'FMCG', marketCap: '₹1.38L Cr', pe: 58.0, rating: 'Buy', qualityScore: 89, analysisSnapshot: 'Household insecticides and personal care innovation driving domestic recovery.', keywords: ['godrej consumer', 'fmcg', 'goodknight', 'cinthol'] },
  { symbol: 'GODREJPROP', name: 'Godrej Properties Limited', exchange: 'NSE', sector: 'Real Estate & Development', marketCap: '₹88,000 Cr', pe: 65.0, rating: 'Strong Buy', qualityScore: 91, analysisSnapshot: 'Tier-1 Indian luxury residential developer delivering record booking value across Mumbai, NCR, and Bengaluru.', keywords: ['godrej properties', 'real estate', 'housing', 'apartments'] },
  { symbol: 'GAIL', name: 'GAIL (India) Limited', exchange: 'NSE', sector: 'Natural Gas Transmission & Petrochem', marketCap: '₹1.45L Cr', pe: 12.8, rating: 'Buy', qualityScore: 89, analysisSnapshot: 'Monopoly interstate gas pipeline network benefiting from India’s natural gas grid expansion.', keywords: ['gail india', 'natural gas', 'pipeline', 'petrochemicals'] },
  { symbol: 'GLENMARK', name: 'Glenmark Pharmaceuticals Limited', exchange: 'NSE', sector: 'Pharmaceuticals', marketCap: '₹45,000 Cr', pe: 32.0, rating: 'Buy', qualityScore: 88, analysisSnapshot: 'Global dermatology and respiratory formulation strength with debt reduction momentum.', keywords: ['glenmark', 'pharma', 'dermatology', 'ryaltris'] },
  { symbol: 'GMRINFRA', name: 'GMR Airports Infrastructure Ltd', exchange: 'NSE', sector: 'Airport Operations & Infrastructure', marketCap: '₹58,000 Cr', pe: 85.0, rating: 'Buy', qualityScore: 87, analysisSnapshot: 'Direct proxy on Indian passenger air traffic growth through Delhi and Hyderabad international airports.', keywords: ['gmr airports', 'delhi airport', 'hyderabad airport', 'aviation infra'] },
  ...
  { symbol: 'ZOMATO', name: 'Zomato Limited', exchange: 'NSE', sector: 'Food Delivery & Quick Commerce', marketCap: '₹2.35L Cr', pe: 145.0, rating: 'Strong Buy', qualityScore: 94, analysisSnapshot: 'Blinkit quick commerce hypergrowth driving explosive revenue and operating leverage.', keywords: ['zomato', 'blinkit', 'quick commerce', 'food delivery'] }
];

export const calculateRelevanceScore = (item, term) => {
  if (!term) return 0;
  ...
  const secWords = sec.split(/[\s,()/-]+/);

  
  if (sym === term) return 100000 + (item.qualityScore || 80);

  
  if (nm === term || kw.includes(term)) return 95000 + (item.qualityScore || 80);

  let score = 0;
  const symStarts = sym.startsWith(term);
  const nmStarts = nm.startsWith(term);
  const kwStarts = kw.some(k => k.startsWith(term));
  const wordStarts = nmWords.some(w => w.startsWith(term)) || kwWords.some(w => w.startsWith(term));
  const secStarts = sec.startsWith(term) || secWords.some(w => w.startsWith(term));

  
  if (symStarts || nmStarts || kwStarts) {
    score += 50000;
    if (symStarts) score += 5000 - sym.length * 50;
    if (kwStarts) score += 4000;
    if (nmStarts) score += 3000;
  }
  ...
  else if (nm.includes(term) || kw.some(k => k.includes(term))) {
    score += 3000;
  }
  
  else if (term.length >= 3 && sec.includes(term)) {
    score += 1000;
  }

  score += (item.qualityScore || 80);
  return score;
};

export const getSmartStockSuggestions = (query, limit = null) => {
  const term = (query || '').trim().toLowerCase();
  if (!term) {
    return limit ? STOCKS_DATABASE.slice(0, limit) : STOCKS_DATABASE.slice(0, 15);
  }

  const matches = STOCKS_DATABASE.filter(item => {
    const sym = (item.symbol || '').toLowerCase();
    const nm = (item.name || '').toLowerCase();
    const sec = (item.sector || '').toLowerCase();
    const kw = (item.keywords || []).map(k => k.toLowerCase());
    ...
    if (symStarts || nmStarts || kwStarts || wordStarts || secStarts) return true;
    if (sym.includes(term) || nm.includes(term) || kw.some(k => k.includes(term))) return true;
    if (term.length >= 3 && sec.includes(term)) return true;

    return false;
  });

  matches.sort((a, b) => calculateRelevanceScore(b, term) - calculateRelevanceScore(a, term));
